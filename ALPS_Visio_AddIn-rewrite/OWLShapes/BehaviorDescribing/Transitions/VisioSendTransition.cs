...
namespace ALPS_Visio_AddIn_rewrite.OWLShapes
{
    public class VisioSendTransition : SendTransition, IVisioExportableWithShape
    {
        ...
        public VisioSendTransition(IState sourceState, IState targetState, string labelForID = null, ITransitionCondition transitionCondition = null, ITransition.TransitionType transitionType = ITransition.TransitionType.Standard, ISet<IDataMappingLocalToOutgoing> dataMappingLocalToOutgoing = null, string comment = null, string additionalLabel = null, IList<IPASSTriple> additionalAttribute = null) : base(sourceState, targetState, labelForID, transitionCondition, transitionType, dataMappingLocalToOutgoing, comment, additionalLabel, additionalAttribute) { export = new TransitionExport(this); }
        ...
        public void ExportToVisio(Visio.Page page)
        {
            ...
            // multiple sends
            VH.SetProperty(export.GetShape(), Constants.Properties.Transition.MultiSendLowerBound, condition.getMultipleLowerBound().ToString());
            VH.SetProperty(export.GetShape(), Constants.Properties.Transition.MultiSendUpperBound, condition.getMultipleUpperBound().ToString());

            // send type
            VH.SetPropertyFormulaU(export.GetShape(), Constants.Properties.Transition.SendType,
                "INDEX(" + (int)condition.getSendType() + ", Prop.sendingType.Format)");
            ...
            if (tempList.Count > 0)
            {
                ...
                VH.SetProperty(export.GetShape(), Constants.Properties.Transition.DataMappingOutgoing, dataMappingString);
            }
        }
